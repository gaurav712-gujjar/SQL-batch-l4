...
select rating,  
count(case
			when rating >= 4.8 then 'high' else 'other' end)
...
select t.driver_id, t.trip_id, t.trip_date, max(fare) from trips t
where t.trip_date =
(select max(trip_date) from trips where driver_id = t.driver_id);
