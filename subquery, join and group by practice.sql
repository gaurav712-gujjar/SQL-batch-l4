...
-- 1.find the employees who earn max salary 
with cte as(
select avg(salary) from employees)
select * from employees where salary > (select * from cte);


-- 2 highest salary --> do'nt use offset and limit in 2 highest salary --> common for interview
-- do'nt use any non aggerate column with aggerate column 

-- subquery to find the max salary then we find second highest salary 
select * from employees where salary =
(select max(salary) from employees where salary < (select max(salary) from employees) );


-- 3.find employees who work it and finance 
-- we use in, any operator in multirow subquery 
select * from employees
where dept_id =any (select dept_id from departments where dept_name= 'it' or dept_name = 'finance');


-- 4.find salary whose salary is greater than all employees in hr departments.(multi row subquery)
select * from employees
...
salary >all (select salary from employees as e join departments as d
...
-- getting employees with the same salary of sales but don't belong to sales ki dedpt_id 
select * from employees where salary in (select salary from cte)
and dept_id not in (select dept_id from cte);


-- corereleted subquery --
-- 6.find employees who earn more than the average saalry of their own departments
select emp_id, emp_name, dept_id, salary from employees as e
...
select * from employees as e join cte 
where e.dept_id=cte.dept_id 
and e.salary > cte.salary;


-- 7.find departments whose average salary is greater than the overall average salary of all employees.
select dept_id, avg(salary) from employees
group by dept_id
having avg(salary) > (select avg(salary) from employees);


-- 8.find the departments with the max total salary.
...
-- 9.find the departments whose min salary is greater than the min salary of the salaes departments.
select dept_id, min(salary) as min_salary from employees
group by dept_id
having min_salary >any (select salary from employees as e join departments as d
...
select dept_id, min(salary) as min_salary from employees
group by dept_id
having min(salary) >any (select salary from employees as e join departments as d
							on e.dept_id = d.dept_id 
							and d.dept_name = 'sales' );
